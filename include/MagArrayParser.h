...
requires(is_SENSOR_TYPE<SENSOR_TYPEs>::value && ...) class MiMedMagnetometerArraySerialConnectionBinary : virtual protected SerialConnection {
   protected:
	...
	void parse_latest_timestamp_message(MessagePart const &message_part) {
		for (int i = static_cast<int>(buffer.size()) - 1; i >= index_timestamp_message + timestamp_message_size - 1; --i) {
			if (int const frame_start = i + 1 - timestamp_message_size, frame_end = i; buffer[frame_end] == 'T' && buffer[frame_start] == 'T') {
				...
				for (int j = 1; j <= timestamp_message_size - 3; ++j) {
	...
	void parse_magnetic_flux_density_messages() {
		for (; index_magnetic_flux_density_message + magnetic_flux_density_message_size <= static_cast<int>(buffer.size()); ++index_magnetic_flux_density_message) {
	...
	void parse_info_messages() {
		...
		if (int const tmp = i - 1 - max_info_message_size + min_info_message_size; tmp > 0) index_info_message = std::max(index_info_message, tmp);
	}
