...
std::expected<void, common::Error> SerialConnection::open_serial_port(std::string const& device) {
	...
	if (_serial_port == -1) {
		auto tmp = common::Error{errno, std::strerror(errno)};
		std::cout << tmp << std::endl;
		return std::unexpected{tmp};
	}
	...
	if (tcgetattr(_serial_port, &tty) != 0) {
		...
		return std::unexpected{common::Error{errno, std::strerror(errno)}};
	}
...
std::expected<void, common::Error> SerialConnection::write_all(std::span<std::uint8_t const> const buffer) {
#ifdef SERIALCONNECTION_USE_BOOST
	boost::system::error_code ec;
	std::size_t n = boost::asio::write(_serial_port, boost::asio::buffer(buffer.data(), buffer.size()), ec);

	if (ec) {
		return std::unexpected(common::Error{ec.value(), ec.message()});
	}
#else
...
std::expected<MessagePart, common::Error> SerialConnection::read_some() {
	...
	return std::unexpected(common::Error{0, "Not connected or a read operation is already in progress."});
}
SerialConnection::~SerialConnection() { close_serial_port(); }
bool SerialConnection::connected() const { return state.load() != ConnectionState::NONE; }
